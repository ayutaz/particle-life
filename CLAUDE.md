...
Particle Life Simulation - 原子状の粒子間の引力・斥力ルールを用いて、自己組織化する生命のようなパターンを生成するシミュレーション。C++とJavaScript (2D/3D)で実装されている。
...
1. openFrameworks v0.12.1 をダウンロードして展開（例: `~/of_v0.12.1_osx_release/`）
2. `particle_life/` を `apps/myApps/` にコピー
3. macOS 用テンプレートをコピー:
   ```
   cp scripts/templates/osx/Makefile apps/myApps/particle_life/
   cp scripts/templates/osx/config.make apps/myApps/particle_life/
   ```
4. ビルド＆実行:
   ```
   cd apps/myApps/particle_life
